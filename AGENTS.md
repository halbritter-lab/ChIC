...
7. **`CONFIG` (`src/config/config.js`) is the single source of truth for constants.** Model params (`CLASS_BASELINE_ML_PER_M: 600`, `GROWTH_RATE_CUTOFFS`), input ranges, and chart Y/X bounds + tick arrays + `CLASS_COLORS` all live there; the domain module and `ChartDisplay.vue` import from it. **Height is never estimated (issue #37):** an import row that lacks a valid, in-range **height, age, or TLV** is **kept but flagged `uncalculable: true`** (`htlv`/`class` null, LGR `'N/A'`), shown in the table as **`N/A`**, and **not plotted** — it is not guessed from a cohort mean (`ASSUMED_HEIGHT_M` and the old `NORMALIZATION_FACTOR: 850` fallback are both gone). The interactive path already requires a valid height, so it never produces an uncalculable row. Add new constants to `CONFIG`, not inline. **External links, repo coordinates, and the contact email live in `src/config/links.js`** (`LINKS`, `buildBugReportUrl`) — de-hardcode UI links there, not inline in components.
