...
Height-adjusted total liver volume is computed as `htTLV = TLV / height` (TLV in ml, height in m). The liver growth rate (LGR, % / year) is an estimate calculated by using the model baseline. The clinical model and all tunable constants are defined in the source (`src/domain/classification.js` and `src/config/config.js`); see [AGENTS.md](../AGENTS.md) for details.
